# ============================================================
#  RIYA JASMIN VASTRAABHARANA - Write All Source Files
#  Run via: run-me.bat  (from inside the project folder)
# ============================================================

Write-Host ""
Write-Host "=============================================" -ForegroundColor Magenta
Write-Host "  Writing all src files..." -ForegroundColor Magenta
Write-Host "=============================================" -ForegroundColor Magenta
Write-Host ""

# Safety check
if (-not (Test-Path "package.json")) {
    Write-Host "ERROR: Run this from inside your project folder!" -ForegroundColor Red
    Write-Host "cd C:\Riya Jasmin Vastraabharana\riya-jasmin-vastraabharana" -ForegroundColor Yellow
    exit 1
}
...
# ── Create folders ──────────────────────────────────────────
Write-Host "[1/3] Creating folders..." -ForegroundColor Cyan

$folders = @(
    "src\types", "src\utils", "src\config",
    "src\store", "src\hooks", "src\pages",
    "src\routes", "src\api", "src\components\common",
    "src\components\layout", "src\components\ui",
    "src\features\auth", "src\features\cart",
    "src\features\checkout", "src\features\products",
    "src\features\orders", "src\features\wishlist",
    "src\features\admin", "src\features\analytics",
    "src\features\whatsapp", "src\ai\fraud",
    "src\ai\recommendations",
    "src\assets\products\sarees\kanjivaram",
    "src\assets\products\sarees\banarasi",
    "src\assets\products\sarees\cotton",
    "src\assets\products\sarees\chanderi",
    "src\assets\products\sarees\mysore-silk",
    "src\assets\products\sarees\patola",
    "src\assets\products\sarees\chiffon",
    "src\assets\products\jewellery\earrings",
    "src\assets\products\jewellery\necklaces",
    "src\assets\products\jewellery\bangles",
    "src\assets\products\jewellery\rings",
    "src\assets\products\jewellery\maang-tikka",
    "src\assets\products\jewellery\anklets",
    "src\assets\products\jewellery\bracelets",
    "src\assets\products\lehengas\bridal",
    "src\assets\products\lehengas\party-wear",
    "src\assets\products\dupattas",
    "src\assets\products\kurtis\anarkali",
    "src\assets\fonts", "src\assets\icons", "src\assets\banners",
    "public\assets"
)
...
# ── Write files ─────────────────────────────────────────────
Write-Host "[2/3] Writing source files..." -ForegroundColor Cyan
...
'@
Write-Host "  + src/types/index.ts" -ForegroundColor DarkGray

# ── src/utils/index.ts ──────────────────────────────────────
Set-Content -Path "src\utils\index.ts" -Encoding UTF8 -Value @'
...
'@
Write-Host "  + src/utils/index.ts" -ForegroundColor DarkGray

# ── src/config/site.ts ──────────────────────────────────────
Set-Content -Path "src\config\site.ts" -Encoding UTF8 -Value @'
...
'@
Write-Host "  + src/config/site.ts" -ForegroundColor DarkGray

# ── src/store/useCartStore.ts ────────────────────────────────
Set-Content -Path "src\store\useCartStore.ts" -Encoding UTF8 -Value @'
...
'@
Write-Host "  + src/store/useCartStore.ts" -ForegroundColor DarkGray

# ── src/store/useWishlistStore.ts ────────────────────────────
Set-Content -Path "src\store\useWishlistStore.ts" -Encoding UTF8 -Value @'
import { create } from 'zustand';
import { persist } from 'zustand/middleware';
...
'@
Write-Host "  + src/store/useWishlistStore.ts" -ForegroundColor DarkGray
...
'@
Write-Host "  + src/index.css" -ForegroundColor DarkGray

# ── src/App.tsx ──────────────────────────────────────────────
Set-Content -Path "src\App.tsx" -Encoding UTF8 -Value @'
    ...
    <div style={{ minHeight: "100vh", background: "#fdf6ef", fontFamily: "'EB Garamond', Georgia, serif", color: "#2c1810" }}>
      ...
      <header style={{ background: "#2c1810", padding: "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 12px rgba(0,0,0,0.4)", flexWrap: "wrap", gap: 8, minHeight: 64 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 32, height: 32, background: "linear-gradient(135deg,#B8860B,#DAA520)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 15, flexShrink: 0 }}>🪷</div>
          <div>
        ...
        <div style={{ display: "flex", gap: 12, alignItems: "center" }}>
          <a href={SITE_CONFIG.social.instagram.url} target="_blank" rel="noreferrer" title="Instagram @rj_alankara" style={{ color: "#d4b896", fontSize: 18, textDecoration: "none" }}>📷</a>
          <a href={SITE_CONFIG.social.facebook.url} target="_blank" rel="noreferrer" title="Facebook rj_alankara" style={{ color: "#d4b896", fontSize: 18, textDecoration: "none" }}>📘</a>
          <a href={SITE_CONFIG.whatsapp.link} target="_blank" rel="noreferrer" title="WhatsApp" style={{ color: "#d4b896", fontSize: 18, textDecoration: "none" }}>💬</a>
      ...
      <div style={{ background: "linear-gradient(135deg,#2c1810 0%,#5a2d0c 50%,#8B1A4A 100%)", padding: "60px 28px", textAlign: "center" }}>
        ...
        <p style={{ color: "#d4b896", fontSize: 16, maxWidth: 440, margin: "0 auto 24px", lineHeight: 1.7 }}>
          Handcrafted sarees & jewellery from India&apos;s finest artisans.<br />Free shipping above ₹1,999
        </p>
        <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
          <button onClick={() => setCategory("sarees")} style={{ background: "#B8860B", border: "none", color: "#fff", padding: "11px 26px", borderRadius: 4, cursor: "pointer", fontFamily: "Cormorant Garamond", fontSize: 16 }}>🥻 Shop Sarees</button>
          <button onClick={() => setCategory("jewellery")} style={{ background: "transparent", border: "1.5px solid #B8860B", color: "#DAA520", padding: "11px 26px", borderRadius: 4, cursor: "pointer", fontFamily: "Cormorant Garamond", fontSize: 16 }}>💎 Shop Jewellery</button>
          <a href={SITE_CONFIG.whatsapp.link} target="_blank" rel="noreferrer" style={{ background: "#25D366", color: "#fff", padding: "11px 26px", borderRadius: 4, fontFamily: "Cormorant Garamond", fontSize: 16, textDecoration: "none", display: "inline-block" }}>💬 WhatsApp Order</a>
        </div>
        <div style={{ display: "flex", justifyContent: "center", gap: 32, marginTop: 28, flexWrap: "wrap" }}>
          <a href={SITE_CONFIG.social.instagram.url} target="_blank" rel="noreferrer" style={{ textAlign: "center", textDecoration: "none" }}><div style={{ fontSize: 20 }}>📷</div><div style={{ fontSize: 12, color: "#B8860B" }}>@rj_alankara</div><div style={{ fontSize: 11, color: "#8B6040" }}>Instagram</div></a>
          <a href={SITE_CONFIG.social.facebook.url} target="_blank" rel="noreferrer" style={{ textAlign: "center", textDecoration: "none" }}><div style={{ fontSize: 20 }}>📘</div><div style={{ fontSize: 12, color: "#B8860B" }}>rj_alankara</div><div style={{ fontSize: 11, color: "#8B6040" }}>Facebook</div></a>
          <div style={{ textAlign: "center" }}><div style={{ fontSize: 20 }}>💬</div><div style={{ fontSize: 12, color: "#B8860B" }}>+91 93810 21541</div><div style={{ fontSize: 11, color: "#8B6040" }}>WhatsApp</div></div>
        </div>
      </div>

      {/* TRUST BADGES */}
      <div style={{ display: "flex", justifyContent: "center", gap: 32, padding: "14px 28px", borderBottom: "1px solid #e8d5bf", flexWrap: "wrap" }}>
        {[["🚚","Free Shipping","Above ₹1,999"],["↩️","7-Day Returns","Easy returns"],["✅","100% Authentic","Handcrafted"],["🔒","Secure Payment","UPI & Cards"],["💬","WhatsApp Support","+91 93810 21541"]].map(([icon,title,sub]) => (
      ...
      <div style={{ display: "flex", justifyContent: "center", gap: 8, padding: "20px 28px 8px" }}>
        {CATEGORIES.map(c => (
          <button key={c.key} onClick={() => setCategory(c.key)} style={{ padding: "8px 22px", borderRadius: 4, border: category === c.key ? "2px solid #B8860B" : "1.5px solid #d4b896", background: category === c.key ? "#B8860B" : "transparent", color: category === c.key ? "#fff" : "#8B6040", cursor: "pointer", fontSize: 14, transition: "all 0.2s" }}>
            {c.key === "all" ? "✨ " : c.key === "sarees" ? "🥻 " : "💎 "}{c.label}
          </button>
        ))}
      </div>
            ...
            <div key={product.id} style={{ background: "#fff", borderRadius: 8, overflow: "hidden", boxShadow: "0 2px 12px rgba(44,24,16,0.08)", transition: "transform 0.2s,box-shadow 0.2s" }}
              ...
              <div style={{ padding: "13px 15px" }}>
                ...
                <div style={{ display: "flex", gap: 7 }}>
                  ...
                  <button onClick={() => handleWhatsApp(product)} title="Order on WhatsApp" style={{ flex: 1, background: "#25D366", color: "#fff", border: "none", padding: "9px 0", borderRadius: 4, cursor: "pointer", fontSize: 15 }}>💬</button>
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 500 }}>
          ...
          <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: 380, background: "#fdf6ef", display: "flex", flexDirection: "column", boxShadow: "-4px 0 24px rgba(0,0,0,0.2)" }}>
            <div style={{ padding: "16px 20px", background: "#2c1810", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <span style={{ fontFamily: "Cormorant Garamond", fontSize: 20, color: "#f5e6d3" }}>🛒 Cart ({itemCount})</span>
              <button onClick={() => setCartOpen(false)} style={{ background: "none", border: "none", color: "#f5e6d3", fontSize: 20, cursor: "pointer" }}>✕</button>
            </div>
            <div style={{ flex: 1, overflowY: "auto", padding: "14px 18px" }}>
              {items.length === 0 ? (
                <div style={{ textAlign: "center", padding: "60px 0", color: "#8B6040" }}><div style={{ fontSize: 48, marginBottom: 12 }}>🛍️</div><div style={{ fontFamily: "Cormorant Garamond", fontSize: 20 }}>Your cart is empty</div></div>
              ) : items.map(item => (
                <div key={item.product.id} style={{ display: "flex", gap: 11, marginBottom: 12, background: "#fff", borderRadius: 8, padding: 11 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                      <button onClick={() => updateQuantity(item.product.id, Math.max(1, item.quantity - 1))} style={{ width: 24, height: 24, border: "1px solid #d4b896", borderRadius: 4, cursor: "pointer", background: "#fff" }}>−</button>
            ...
            {items.length > 0 && (
              <div style={{ padding: "16px 18px", borderTop: "1px solid #e8d5bf" }}>
                ...
                <button onClick={() => { setCartOpen(false); setCheckoutOpen(true); }} style={{ width: "100%", background: "#B8860B", color: "#fff", border: "none", padding: "12px 0", borderRadius: 4, cursor: "pointer", fontFamily: "Cormorant Garamond", fontSize: 16, marginBottom: 8 }}>Proceed to Checkout →</button>
                <a href={SITE_CONFIG.whatsapp.link + "?text=" + encodeURIComponent("Hi! I want to order:\n" + items.map(i => "- " + i.product.name + " x" + i.quantity).join("\n") + "\nTotal: " + formatCurrency(grandTotal))} target="_blank" rel="noreferrer"
                  style={{ display: "block", width: "100%", background: "#25D366", color: "#fff", padding: "10px 0", borderRadius: 4, textAlign: "center", textDecoration: "none", fontSize: 14, boxSizing: "border-box" }}>💬 Order via WhatsApp</a>
              </div>
            )}
          </div>
        </div>
            ...
            {orderDone ? (
              <div style={{ padding: "48px 32px", textAlign: "center" }}>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                  <a href={SITE_CONFIG.social.instagram.url} target="_blank" rel="noreferrer" style={{ flex: 1, background: "#833AB4", color: "#fff", padding: "9px 0", borderRadius: 6, textAlign: "center", textDecoration: "none", fontSize: 12 }}>📷 Follow @rj_alankara</a>
                  <a href={SITE_CONFIG.social.facebook.url} target="_blank" rel="noreferrer" style={{ flex: 1, background: "#1877F2", color: "#fff", padding: "9px 0", borderRadius: 6, textAlign: "center", textDecoration: "none", fontSize: 12 }}>📘 Like on Facebook</a>
                </div>
              ...
              <>
                <div style={{ padding: "16px 22px", background: "#2c1810", borderRadius: "12px 12px 0 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                  ...
                  <div style={{ display: "flex", gap: 5, alignItems: "center" }}>
                    ...
                    <button onClick={resetCheckout} style={{ background: "none", border: "none", color: "#f5e6d3", fontSize: 18, cursor: "pointer", marginLeft: 8 }}>✕</button>
                  </div>
                </div>
                <div style={{ padding: "22px" }}>
                  {payStep === 1 && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                      ...
                      <div style={{ background: "#fff", borderRadius: 8, padding: "12px 14px", border: "1px solid #e8d5bf" }}>
                        ...
                        {items.map(i => <div key={i.product.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#8B6040", marginBottom: 3 }}><span>{i.product.name} × {i.quantity}</span><span>{formatCurrency(i.product.price * i.quantity)}</span></div>)}
                  ...
                  {payStep === 2 && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                      <div style={{ display: "flex", gap: 7 }}>
                        {([["upi","📱 UPI"],["card","💳 Card"],["cod","🏠 COD"]] as [PaymentMethod,string][]).map(([val,label]) => (
                      ...
                      {payMethod === "upi" && (
                        <div>
                          ...
                          <div style={{ display: "flex", gap: 7, marginTop: 10 }}>
                            {["📲 GPay","💰 PhonePe","💙 Paytm","🏛️ BHIM"].map(a => <div key={a} style={{ flex: 1, textAlign: "center", fontSize: 11, background: "#fff", border: "1px solid #e8d5bf", borderRadius: 6, padding: "7px 3px", color: "#8B6040" }}>{a}</div>)}
                          </div>
                        </div>
                      )}
                      {payMethod === "card" && (
                      ...
                      {payMethod === "cod" && (
                        <div style={{ background: "#fff8ee", border: "1px solid #e8d5bf", borderRadius: 8, padding: "16px", textAlign: "center", color: "#8B6040" }}>
                          ...
                          <div style={{ fontSize: 13 }}>Pay {formatCurrency(grandTotal + 49)} on delivery<br />(includes ₹49 COD fee)</div>
                        </div>
                      )}
                      <div style={{ background: "#e8f5e9", borderRadius: 6, padding: "9px 13px", fontSize: 12, color: "#2E7D5C" }}>🔒 100% secure. SSL encrypted.</div>
        ...
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 28, marginBottom: 28 }}>
            ...
            <div>
              ...
              <a href={SITE_CONFIG.whatsapp.link} target="_blank" rel="noreferrer" style={{ display: "block", color: "#25D366", textDecoration: "none", fontSize: 13, lineHeight: 2 }}>💬 WhatsApp: +91 93810 21541</a>
              <a href={"mailto:" + SITE_CONFIG.email} style={{ display: "block", color: "#d4b896", textDecoration: "none", fontSize: 13 }}>✉️ {SITE_CONFIG.email}</a>
            </div>
            <div>
              <div style={{ fontFamily: "Cormorant Garamond", fontSize: 15, color: "#f5e6d3", marginBottom: 10 }}>Follow Us</div>
              <a href={SITE_CONFIG.social.instagram.url} target="_blank" rel="noreferrer" style={{ display: "block", color: "#E1306C", textDecoration: "none", fontSize: 13, lineHeight: 2 }}>📷 @rj_alankara</a>
              <a href={SITE_CONFIG.social.facebook.url} target="_blank" rel="noreferrer" style={{ display: "block", color: "#1877F2", textDecoration: "none", fontSize: 13 }}>📘 rj_alankara</a>
            </div>
          </div>
          <div style={{ borderTop: "1px solid #3a2010", paddingTop: 16, textAlign: "center", fontSize: 11, color: "#5a3a2a" }}>
            © 2025 Riya Jasmin Vastraabharana. All rights reserved. Handcrafted with ❤️ in India.
          </div>
        </div>
      </footer>
    </div>
...
'@
Write-Host "  + src/App.tsx" -ForegroundColor DarkGray

# ── Catalog JSONs ────────────────────────────────────────────
$earringsCatalog = '[{"id":"earring-001","sku":"RJV-EAR-001","name":"Pearl Drop Earrings","slug":"pearl-drop-earrings","price":3299,"originalPrice":5000,"images":["pearl-drop.jpg"],"tag":"Trending","desc":"Freshwater pearls, sterling silver base","stock":24,"material":"Sterling Silver","occasion":["Wedding","Festive","Daily"],"whatsapp":"Hi! I want to order Pearl Drop Earrings (RJV-EAR-001) Rs 3299"}]'
Set-Content -Path "src\assets\products\jewellery\earrings\catalog.json" -Encoding UTF8 -Value $earringsCatalog

$kanjiCatalog = '[{"id":"saree-kan-001","sku":"RJV-SAR-KAN-001","name":"Ruby Red Kanjivaram Silk","slug":"ruby-red-kanjivaram","price":8499,"originalPrice":12000,"images":["kanjivaram-red.jpg"],"tag":"Bestseller","desc":"Pure Zari work bridal Kanjivaram. Handwoven by master weavers.","stock":8,"material":"Pure Mulberry Silk","occasion":["Wedding","Bridal","Festive"],"whatsapp":"Hi! I want to order Ruby Red Kanjivaram (RJV-SAR-KAN-001) Rs 8499"}]'
Set-Content -Path "src\assets\products\sarees\kanjivaram\catalog.json" -Encoding UTF8 -Value $kanjiCatalog

$cottonCatalog = '[{"id":"saree-cot-001","sku":"RJV-SAR-COT-001","name":"Sage Green Chanderi Cotton","slug":"sage-green-chanderi","price":2799,"originalPrice":4200,"images":["chanderi-green.jpg"],"tag":"Sale","desc":"Lightweight Chanderi cotton. Perfect for office and casual wear.","stock":30,"material":"Chanderi Cotton","occasion":["Daily","Office","Casual"],"whatsapp":"Hi! I want to order Sage Green Chanderi (RJV-SAR-COT-001) Rs 2799"}]'
Set-Content -Path "src\assets\products\sarees\cotton\catalog.json" -Encoding UTF8 -Value $cottonCatalog

Write-Host "  + catalog.json files" -ForegroundColor DarkGray
Write-Host ""

# ── Step 3: Git commit ───────────────────────────────────────
Write-Host "[3/3] Git commit..." -ForegroundColor Cyan
git add . 2>&1 | Out-Null
git commit -m "feat: add all src files - App, store, types, config, utils" 2>&1 | Out-Null
...
Write-Host "  You will see your full website!" -ForegroundColor Cyan
Write-Host ""
