...
// ═══════════════════════════════════════════════════════════════
//  HOW TO ADD IMAGES & VIDEOS — READ THIS!
//
//  STEP 1: Put ALL your files in this folder:
//    C:\Riya Jasmin Vastraabharana\riya-jasmin-vastraabharana\public\
//
//  STEP 2: In the PRODUCTS list below, use just the filename:
//    images: ["/my-saree.jpg"]          ← image
//    images: ["/saree1.jpg", "/saree2.jpg"]  ← multiple images
//    video:  "/my-saree-video.mp4"      ← video (optional)
//
//  SUPPORTED FORMATS:
...
//
//  VIDEO BEHAVIOUR:
//    - Video plays SILENTLY when customer hovers on product card
//    - Falls back to image when not hovering
//    - A small "VIDEO" badge appears on the card
//
//  BULK IMAGES — How to add 20 products at once:
//    1. Copy all images into public\ folder at once (Ctrl+A, Copy, Paste)
//    2. Add one entry per product below, copy-paste the template
//    3. npm run dev — all appear instantly!
// ═══════════════════════════════════════════════════════════════

interface ProductWithMedia extends Product {
  video?: string;          // optional video file, e.g. "/saree-video.mp4"
  images2?: string[];      // second+ images shown on hover (if no video)
}

const PRODUCTS: ProductWithMedia[] = [
  // ── JEWELLERY ──────────────────────────────────────────────
  {
    id: "e001", sku: "RJV-EAR-001",
    ...
    // video: "/pearl-earrings-video.mp4",  ← uncomment and add your video
  ...
  {
    id: "e002", sku: "RJV-EAR-002",
    name: "Kundan Jhumkas", slug: "kundan-jhumkas",
  ...
  {
    id: "n001", sku: "RJV-NEC-001",
    name: "Temple Gold Necklace", slug: "temple-gold-necklace",
  ...
  {
    id: "b001", sku: "RJV-BAN-001",
    name: "Polki Diamond Bangles", slug: "polki-diamond-bangles",
  ...
  // ── SAREES ─────────────────────────────────────────────────
  {
    id: "s001", sku: "RJV-SAR-KAN-001",
    name: "Ruby Red Kanjivaram", slug: "ruby-red-kanjivaram",
    price: 8499, originalPrice: 12000,
    images: ["https://images.unsplash.com/photo-1610030469983-98e550d6193c?w=400&q=80"],
    // video: "/kanjivaram-red-video.mp4",  ← add your own video here
  ...
  {
    id: "s002", sku: "RJV-SAR-BAN-001",
    name: "Royal Blue Banarasi", slug: "royal-blue-banarasi",
  ...
  },
  {
    ...
    name: "Sage Green Chanderi Cotton", slug: "sage-green-chanderi",
  ...
  {
    id: "s004", sku: "RJV-SAR-MYS-001",
    name: "Mysore Crepe Silk", slug: "mysore-crepe-silk",
  ...
  // ══════════════════════════════════════════════════════════
  //  ADD YOUR OWN PRODUCTS HERE — copy this template:
  //
  // {
  //   id: "s005",  ← must be unique
  //   sku: "RJV-SAR-KAN-002",
  ...
  //   category: "sarees",       ← sarees / jewellery
  //   subcategory: "kanjivaram", ← kanjivaram / banarasi / cotton / earrings / etc.
  //   whatsapp: "Hi! I want to order Blue Kanjivaram (RJV-SAR-KAN-002) Rs 8999"
  // },
  // ══════════════════════════════════════════════════════════
];

// ── Featured videos shown on homepage video section ──────────
// Put your .mp4 files in public\ folder and list them here
const FEATURED_VIDEOS = [
  // { src: "/collection-reel.mp4", title: "Festive Collection 2025", thumb: "/collection-thumb.jpg" },
  // { src: "/bridal-sarees.mp4",   title: "Bridal Sarees",           thumb: "/bridal-thumb.jpg"    },
  // { src: "/jewellery-reel.mp4",  title: "Gold Jewellery",          thumb: "/jewellery-thumb.jpg" },
];

// ── Payment details ──────────────────────────────────────────
const PAYMENT_DETAILS = {
  upiId:         "billa14673370@ybl",
  phonePe:       "9381021541",
  googlePay:     "9381021541",
  paytm:         "9381021541",
  bankName:      "Union Bank of India",
  accountName:   "Abdul Sattar Billa",
  accountNumber: "178710100122045",
  ifsc:          "UBIN0817856",
};

const TAG_COLORS: Record<string, string> = {
  Bestseller: "#B8860B", New: "#2E7D5C", Sale: "#C0392B",
  Premium: "#6B1A8B", Trending: "#1A3A6B",
};
...
const TICKER_MESSAGES = [
  ...
  "UPI: " billa14673370@ybl,9381021541"
];
...
function Ticker() {
  ...
  return (
    <div style={{ background: "#B8860B", color: "#fff", height: 34, display: "flex", alignItems: "center", overflow: "hidden" }}>
      ...
      `}</style>
      <div className="ticker-track" style={{ fontSize: 13, fontFamily: "'EB Garamond',Georgia,serif", letterSpacing: 0.5 }}>
        <span>{text}{text}</span>
      </div>
    </div>
  );
...
function ProductCard({
  ...
  const hasVideo = !!product.video && !videoError;
  ...
  return (
    <div
      ...
      style={{ background: "#fff", borderRadius: 8, overflow: "hidden", boxShadow: "0 2px 12px rgba(44,24,16,0.08)", transition: "transform 0.2s,box-shadow 0.2s", transform: hovering ? "translateY(-4px)" : "none", boxShadow: hovering ? "0 8px 28px rgba(44,24,16,0.16)" : "0 2px 12px rgba(44,24,16,0.08)" } as React.CSSProperties}
    >
      {/* ── Media area ── */}
      <div style={{ position: "relative", height: 220, overflow: "hidden", background: "#f0e6d8" }}>
        ...
        <img
          ...
          alt={product.name}
          style={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform 0.4s, opacity 0.3s", transform: (hovering && !hasVideo) ? "scale(1.06)" : "none", opacity: (hovering && hasVideo) ? 0 : 1, position: "absolute", inset: 0 }}
          onError={e => { (e.currentTarget as HTMLImageElement).src = "https://images.unsplash.com/photo-1583391733956-6c78276477e4?w=400&q=80"; }}
        />

        {/* Video — only rendered if product has video */}
        {hasVideo && (
          <video
            ref={videoRef}
            src={product.video}
            ...
            style={{ width: "100%", height: "100%", objectFit: "cover", position: "absolute", inset: 0, opacity: hovering ? 1 : 0, transition: "opacity 0.3s" }}
          />
        )}

        {/* Video badge */}
        {hasVideo && !hovering && (
          <div style={{ position: "absolute", bottom: 8, right: 8, background: "rgba(0,0,0,0.65)", color: "#fff", fontSize: 10, fontWeight: 700, padding: "3px 7px", borderRadius: 3, letterSpacing: 1 }}>
            VIDEO
          </div>
        )}
        ...
        {/* Tag */}
        <div style={{ position: "absolute", top: 9, left: 9, background: TAG_COLORS[product.tag] || "#B8860B", color: "#fff", padding: "3px 9px", borderRadius: 3, fontSize: 11, fontWeight: 600, zIndex: 2 }}>
          {product.tag}
        </div>

        {/* Wishlist */}
        <button onClick={e => { e.stopPropagation(); onWishlist(); }}
          style={{ position: "absolute", top: 7, right: 7, background: "rgba(255,255,255,0.92)", border: "none", width: 30, height: 30, borderRadius: "50%", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 2 }}>
      ...
      <div style={{ padding: "13px 15px" }}>
        ...
        <h3 style={{ fontFamily: "Cormorant Garamond", fontSize: 17, margin: "0 0 4px", color: "#2c1810" }}>{product.name}</h3>
        <p style={{ fontSize: 12, color: "#8B6040", margin: "0 0 6px", lineHeight: 1.5 }}>{product.desc}</p>
        {product.material && <div style={{ fontSize: 11, color: "#aaa", marginBottom: 8 }}>{product.material}</div>}
        <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 11 }}>
          <span style={{ fontSize: 18, fontWeight: 600, color: "#2c1810" }}>{formatCurrency(product.price)}</span>
        ...
        <div style={{ display: "flex", gap: 7 }}>
          ...
          <button onClick={onWhatsApp} title="Order on WhatsApp"
...
function VideoSection() {
  ...
  return (
    <div style={{ padding: "40px 28px", background: "#2c1810" }}>
      <div style={{ textAlign: "center", marginBottom: 28 }}>
        <div style={{ fontSize: 11, letterSpacing: 5, color: "#B8860B", textTransform: "uppercase", marginBottom: 8 }}>Collection Reels</div>
        <h2 style={{ fontFamily: "Cormorant Garamond", fontSize: 32, color: "#f5e6d3", fontWeight: 300, margin: 0 }}>Watch Our Latest</h2>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 20, maxWidth: 1000, margin: "0 auto" }}>
        {FEATURED_VIDEOS.map((v, i) => (
          <div key={i} style={{ borderRadius: 8, overflow: "hidden", position: "relative", background: "#1a0e08" }}>
            <video
            ...
            {playing !== i && (
              <div onClick={() => handlePlay(i)}
                style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", cursor: "pointer", background: "rgba(0,0,0,0.3)" }}>
                <div style={{ width: 56, height: 56, borderRadius: "50%", background: "rgba(184,134,11,0.9)", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 10 }}>
                  <svg viewBox="0 0 24 24" width="24" height="24" fill="white"><polygon points="8,5 19,12 8,19"/></svg>
                </div>
                <div style={{ fontFamily: "Cormorant Garamond", fontSize: 16, color: "#f5e6d3", textAlign: "center" }}>{v.title}</div>
              </div>
            )}
          </div>
        ))}
...
export default function App() {
  ...
  const handleAddToCart = (p: ProductWithMedia) => { addItem(p); showToast(p.name + " added to cart!"); };
  const handleWhatsApp = (p: ProductWithMedia) => {
  ...
  const resetCheckout = () => {
    setCheckoutOpen(false); setPayStep(1); setOrderDone(""); setPaymentConfirmed(false);
    ...
    <div style={{ minHeight: "100vh", background: "#fdf6ef", fontFamily: "'EB Garamond',Georgia,serif", color: "#2c1810" }}>
      ...
      <header style={{ background: "#2c1810", padding: "0 28px", display: "flex", alignItems: "center", justifyContent: "space-between", position: "sticky", top: 0, zIndex: 100, boxShadow: "0 2px 12px rgba(0,0,0,0.4)", gap: 12, minHeight: 64 }}>
        ...
        <div style={{ display: "flex", gap: 14, alignItems: "center", flexShrink: 0 }}>
          <a href={SITE_CONFIG.social.instagram.url} target="_blank" rel="noreferrer" style={{ textDecoration: "none", display: "flex" }}><IconInstagram /></a>
          <a href={SITE_CONFIG.social.facebook.url} target="_blank" rel="noreferrer" style={{ textDecoration: "none", display: "flex" }}><IconFacebook /></a>
          <a href={SITE_CONFIG.whatsapp.link} target="_blank" rel="noreferrer" style={{ textDecoration: "none", display: "flex" }}><IconWhatsApp /></a>
      ...
      <div style={{ background: "linear-gradient(135deg,#2c1810 0%,#5a2d0c 50%,#8B1A4A 100%)", padding: "64px 28px", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", gap: 10, justifyContent: "center", flexWrap: "wrap" }}>
          <button onClick={() => handleCategoryClick("sarees")} style={{ background: "#B8860B", border: "none", color: "#fff", padding: "12px 28px", borderRadius: 4, cursor: "pointer", fontFamily: "Cormorant Garamond", fontSize: 16 }}>Shop Sarees</button>
          <button onClick={() => handleCategoryClick("jewellery")} style={{ background: "transparent", border: "2px solid #B8860B", color: "#DAA520", padding: "12px 28px", borderRadius: 4, cursor: "pointer", fontFamily: "Cormorant Garamond", fontSize: 16 }}>Shop Jewellery</button>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 36, marginTop: 32, flexWrap: "wrap" }}>
          <a href={SITE_CONFIG.social.instagram.url} target="_blank" rel="noreferrer" style={{ textAlign: "center", textDecoration: "none" }}><div style={{ display: "flex", justifyContent: "center", marginBottom: 4 }}><IconInstagram size={22} /></div><div style={{ fontSize: 12, color: "#B8860B" }}>@rj_alankara</div><div style={{ fontSize: 11, color: "#8B6040" }}>Instagram</div></a>
          <a href={SITE_CONFIG.social.facebook.url} target="_blank" rel="noreferrer" style={{ textAlign: "center", textDecoration: "none" }}><div style={{ display: "flex", justifyContent: "center", marginBottom: 4 }}><IconFacebook size={22} /></div><div style={{ fontSize: 12, color: "#B8860B" }}>rj_alankara</div><div style={{ fontSize: 11, color: "#8B6040" }}>Facebook</div></a>
          <a href={SITE_CONFIG.whatsapp.link} target="_blank" rel="noreferrer" style={{ textAlign: "center", textDecoration: "none" }}><div style={{ display: "flex", justifyContent: "center", marginBottom: 4 }}><IconWhatsApp size={22} /></div><div style={{ fontSize: 12, color: "#B8860B" }}>+91 93810 21541</div><div style={{ fontSize: 11, color: "#8B6040" }}>WhatsApp</div></a>
        </div>
      </div>

      {/* TRUST BADGES */}
      <div style={{ display: "flex", justifyContent: "center", gap: 36, padding: "14px 28px", borderBottom: "1px solid #e8d5bf", flexWrap: "wrap" }}>
        {[["Free Shipping","Above Rs. 1,999"],["7-Day Returns","Easy returns"],["100% Authentic","Handcrafted"],["Secure Payment","UPI & Cards"],["WhatsApp Support","+91 93810 21541"]].map(([t,s]) => (
          <div key={t} style={{ textAlign: "center" }}><div style={{ fontSize: 12, fontWeight: 600 }}>{t}</div><div style={{ fontSize: 11, color: "#8B6040" }}>{s}</div></div>
        ))}
      </div>

      {/* FEATURED VIDEOS (shows only if you add videos to FEATURED_VIDEOS array above) */}
      <VideoSection />
      ...
      <div id="products-section" style={{ display: "flex", justifyContent: "center", gap: 8, padding: "20px 28px 8px" }}>
        {CATEGORIES.map(c => (
          <button key={c.key} onClick={() => setCategory(c.key)}
            style={{ padding: "9px 24px", borderRadius: 4, border: category === c.key ? "2px solid #B8860B" : "1.5px solid #d4b896", background: category === c.key ? "#B8860B" : "transparent", color: category === c.key ? "#fff" : "#8B6040", cursor: "pointer", fontSize: 14, fontWeight: category === c.key ? 600 : 400 }}>
            {c.label}
          </button>
        ))}
      </div>
      ...
      <div style={{ padding: "16px 28px 48px" }}>
        <div style={{ marginBottom: 16, color: "#8B6040", fontSize: 13 }}>
          Showing {filtered.length} {category !== "all" ? category : "products"}{search ? ` matching "${search}"` : ""}
        </div>
          ...
          <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: 380, background: "#fdf6ef", display: "flex", flexDirection: "column", boxShadow: "-4px 0 24px rgba(0,0,0,0.2)" }}>
            ...
            {items.length > 0 && (
              <div style={{ padding: "16px 18px", borderTop: "1px solid #e8d5bf" }}>
                ...
                <button onClick={() => { setCartOpen(false); setCheckoutOpen(true); }} style={{ width: "100%", background: "#B8860B", color: "#fff", border: "none", padding: "13px 0", borderRadius: 4, cursor: "pointer", fontFamily: "Cormorant Garamond", fontSize: 17, marginBottom: 8 }}>Proceed to Checkout</button>
          ...
          <div style={{ position: "relative", background: "#fdf6ef", borderRadius: 12, width: "100%", maxWidth: 500, maxHeight: "93vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(0,0,0,0.4)" }}>
            {orderDone ? (
              <div style={{ padding: "48px 32px", textAlign: "center" }}>
                <div style={{ width: 60, height: 60, background: "#e8f5e9", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 16px" }}>
                  <svg viewBox="0 0 24 24" width="30" height="30" fill="none" stroke="#2E7D5C" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>
                </div>
                ...
                <p style={{ color: "#8B6040", lineHeight: 1.7, marginBottom: 16 }}>Thank you, {form.name}!<br />WhatsApp confirmation opening...</p>
                ...
                <div style={{ display: "flex", gap: 8, marginBottom: 14 }}>
                  <a href={SITE_CONFIG.social.instagram.url} target="_blank" rel="noreferrer" style={{ flex: 1, background: "#833AB4", color: "#fff", padding: "9px 0", borderRadius: 6, textAlign: "center", textDecoration: "none", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}><IconInstagram size={13} /> Follow on Instagram</a>
                  <a href={SITE_CONFIG.social.facebook.url} target="_blank" rel="noreferrer" style={{ flex: 1, background: "#1877F2", color: "#fff", padding: "9px 0", borderRadius: 6, textAlign: "center", textDecoration: "none", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}><IconFacebook size={13} /> Like on Facebook</a>
                </div>
                ...
                <div style={{ padding: "22px" }}>
                  {payStep === 1 && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                      ...
                      <button onClick={() => { if (form.name && form.phone && form.address && form.city && form.state && form.pincode) setPayStep(2); else showToast("Please fill all required fields"); }} style={{ width: "100%", background: "#B8860B", color: "#fff", border: "none", padding: "13px 0", borderRadius: 4, cursor: "pointer", fontFamily: "Cormorant Garamond", fontSize: 17 }}>Continue to Payment</button>
                    </div>
                  )}
                  {payStep === 2 && (
                    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                      <div style={{ display: "flex", gap: 7 }}>
                        {([["upi","UPI / QR"],["card","Card"],["cod","Cash on Delivery"]] as [PaymentMethod,string][]).map(([val,label]) => (
                          <button key={val} onClick={() => setPayMethod(val)} style={{ flex: 1, padding: "9px 4px", border: payMethod === val ? "2px solid #B8860B" : "1.5px solid #d4b896", borderRadius: 6, background: payMethod === val ? "#fff8ee" : "#fff", cursor: "pointer", fontSize: 12, color: payMethod === val ? "#B8860B" : "#8B6040", fontWeight: payMethod === val ? 600 : 400 }}>{label}</button>
                        ))}
                      </div>
                      {payMethod === "upi" && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                          ...
                          <div style={{ background: "#fff", borderRadius: 8, padding: "16px", border: "1px solid #e8d5bf" }}>
                            <div style={{ fontFamily: "Cormorant Garamond", fontSize: 16, marginBottom: 12 }}>Pay via UPI</div>
                            <div style={{ background: "#fff8ee", borderRadius: 6, padding: "10px 14px", marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center", border: "1px dashed #B8860B" }}>
                              <div><div style={{ fontSize: 11, color: "#8B6040", marginBottom: 2 }}>UPI ID</div><div style={{ fontWeight: 700, fontSize: 16, fontFamily: "monospace" }}>{PAYMENT_DETAILS.upiId}</div></div>
                              <button onClick={() => { navigator.clipboard.writeText(PAYMENT_DETAILS.upiId); showToast("UPI ID copied!"); }} style={{ background: "#B8860B", color: "#fff", border: "none", padding: "6px 12px", borderRadius: 4, cursor: "pointer", fontSize: 12 }}>Copy</button>
                            </div>
                            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 12 }}>
                              {[{name:"Google Pay",color:"#4285F4"},{name:"PhonePe",color:"#5f259f"},{name:"Paytm",color:"#00BAF2"},{name:"BHIM",color:"#FF6B35"}].map(app => (
                                <div key={app.name} style={{ background: app.color, color: "#fff", padding: "10px 8px", borderRadius: 6, textAlign: "center", fontSize: 13, fontWeight: 600 }}>
                                  {app.name}<br /><span style={{ fontSize: 11, fontWeight: 400 }}>{PAYMENT_DETAILS.upiId}</span>
                                </div>
                              ))}
                            </div>
                            <div style={{ background: "#f9f5f0", borderRadius: 6, padding: "10px 12px", fontSize: 12, color: "#8B6040" }}>
                              <div style={{ fontWeight: 600, color: "#2c1810", marginBottom: 4 }}>Bank Transfer</div>
                          ...
                          <div style={{ background: "#e8f5e9", borderRadius: 8, padding: "14px", border: "1px solid #c8e6c9" }}>
                            <div style={{ fontSize: 13, color: "#2E7D5C", fontWeight: 600, marginBottom: 5 }}>After paying:</div>
                            <div style={{ fontSize: 12, color: "#2E7D5C", lineHeight: 1.7 }}>Send payment screenshot to WhatsApp +91 93810 21541<br />Order confirmed within 1 hour.</div>
                            <label style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 10, cursor: "pointer" }}>
                              <input type="checkbox" checked={paymentConfirmed} onChange={e => setPaymentConfirmed(e.target.checked)} style={{ width: 16, height: 16, cursor: "pointer" }} />
                          ...
                          <div style={{ display: "flex", gap: 9 }}>
                            ...
                            <button onClick={handlePlaceOrder} disabled={!paymentConfirmed} style={{ flex: 2, background: paymentConfirmed ? "#2E7D5C" : "#ccc", color: "#fff", border: "none", padding: "11px 0", borderRadius: 4, cursor: paymentConfirmed ? "pointer" : "not-allowed", fontFamily: "Cormorant Garamond", fontSize: 16 }}>Confirm Order</button>
                          </div>
                        </div>
                      )}
                      {payMethod === "card" && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                          <div style={{ background: "#fff8ee", borderRadius: 8, padding: "12px 14px", fontSize: 13, color: "#B8860B", border: "1px solid #e8d5bf" }}>Razorpay card payments coming soon! Please use UPI for now.</div>
                      ...
                      {payMethod === "cod" && (
                        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                          <div style={{ background: "#fff8ee", border: "1px solid #e8d5bf", borderRadius: 8, padding: "20px", textAlign: "center" }}>
                            <div style={{ fontFamily: "Cormorant Garamond", fontSize: 20, marginBottom: 6 }}>Cash on Delivery</div>
                            <div style={{ fontSize: 28, fontWeight: 700, color: "#B8860B", marginBottom: 6 }}>{formatCurrency(grandTotal + 49)}</div>
                            <div style={{ fontSize: 13, color: "#8B6040" }}>Includes Rs. 49 COD fee. Pay when order arrives.</div>
                          </div>
                          <div style={{ display: "flex", gap: 9 }}>
                            <button onClick={() => setPayStep(1)} style={{ flex: 1, background: "#fff", color: "#2c1810", border: "1.5px solid #d4b896", padding: "11px 0", borderRadius: 4, cursor: "pointer", fontFamily: "Cormorant Garamond", fontSize: 15 }}>Back</button>
                            <button onClick={handlePlaceOrder} style={{ flex: 2, background: "#2E7D5C", color: "#fff", border: "none", padding: "11px 0", borderRadius: 4, cursor: "pointer", fontFamily: "Cormorant Garamond", fontSize: 16 }}>Place Order</button>
        ...
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(180px,1fr))", gap: 28, marginBottom: 28 }}>
            <div>
              ...
              <div style={{ marginTop: 10, fontSize: 12, color: "#8B6040" }}>
                <div>UPI: {PAYMENT_DETAILS.upiId}</div>
